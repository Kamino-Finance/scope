    ...
    'a: 'b,
{
    ...
    let vault = {
        ...
        SplTokenAccount::unpack(&vault_borrow)
    }?;

    let price_data: PriceData = account_deserialize(price_info)?;

    let rate = get_rate(redstone_price_data, &mint)?;
...
fn check_accounts(
    ...
    if state.asset_vault != *vault_account
        ...
        || *redstone_adapter_account.owner != state.nav_provider_program
    {
...
fn get_rate(price_info: &AccountInfo, mint: &Mint) -> ScopeResult<u64> {
    let price_data: PriceData = account_deserialize(price_info)?;
